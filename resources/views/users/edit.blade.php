      ...
      <div class="mt-10 grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6">
        ...
        <div class="sm:col-span-4">
          ...
          <div class="mt-2 flex items-center gap-x-3">
            <img src="{{ Str::startsWith(auth()->user()->image, 'storage/') ? asset(auth()->user()->image) : auth()->user()->image }}" class="h-12 w-12 rounded-full mr-5 border border-gray-300">
            <input type="file" name="image" id="file_input" class="w-full border border-gray-200 bg-gray-50 block rounded-xl focus:outline-none">
          </div>
          @error('image')
            <div class="mt-2 text-sm text-red-600">{{$message}}</div>
          @enderror
