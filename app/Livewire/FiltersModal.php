...
class FiltersModal extends ModalComponent
{
    ...
    public function filter_clarendon(){
        ...
        // read an image
         $path = storage_path('app' . DIRECTORY_SEPARATOR . 'public' . DIRECTORY_SEPARATOR . $this->image);
    ...
    public function filter_gingham(){
        $manager = new ImageManager(new Driver());
         $path = storage_path('app' . DIRECTORY_SEPARATOR . 'public' . DIRECTORY_SEPARATOR . $this->image);
         $img = $manager->read($path);
        $img = $img->brightness(10)->contrast(5)->colorize(10, 0, -10)->gamma(1.1);
        $filteredFileName = Str::random(20) . ".jpeg";
        $filteredImagePath = 'temp/' . $filteredFileName;
        $img->save(storage_path('app/public/' . $filteredImagePath));
        $this->filtered_image =  $filteredImagePath;
    ...
    public function filter_perpetua(){
        $manager = new ImageManager(new Driver());
         $path = storage_path('app' . DIRECTORY_SEPARATOR . 'public' . DIRECTORY_SEPARATOR . $this->image);
         $img = $manager->read($path);
        $img = $img->colorize(-30, 10, 10)->contrast(-10)->gamma(0.9);
        $filteredFileName = Str::random(20) . ".jpeg";
        $filteredImagePath = 'temp/' . $filteredFileName;
        $img->save(storage_path('app/public/' . $filteredImagePath));
        $this->filtered_image =  $filteredImagePath;
